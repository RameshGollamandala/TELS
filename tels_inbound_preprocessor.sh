...
#   standard ODI files from OIC Pre-Processor files.
#
# Invoked By: LND Autoloader based on the mapping of file-type to file handler
#   script name.
#
# Command Line: tels_inbound_preprocessor.sh <inbound filename without path>
#
################################################################################
...
#if [ $srcfiletype = "CCB-COMM_PREPROCCOMMISSIONS" -o $srcfiletype = "SOURCE_FILETYPE2" ]; then
if [ $srcfiletype = "CCB-COMM_PREPROCCOMMISSIONS" -o $srcfiletype = "OIC_PREPROC" ]; then
...
if [ $srcfiletype = "CCB-COMM_PREPROCCOMMISSIONS" ]; then
    parameterfile="${tenantid_uc}_CCB-COMM_PREPROCCOMMISSIONS_parm.txt"
elif [ $srcfiletype = "OIC_PREPROC" ]; then
    parameterfile="${tenantid_uc}_OIC_PREPROC_parm.txt"
fi
...
#        MoveFilesToAppServer $infatgtinbound $OutputFileName3$compdate".txt" $target_archive_dir $OutputFileName3_wftype
    fi
    if [ "$OutputFileName4" != "" ]; then

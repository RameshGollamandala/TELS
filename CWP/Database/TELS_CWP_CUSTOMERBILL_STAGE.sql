DROP TABLE TELS_CWP_CUSTOMERBILL_STAGE CASCADE CONSTRAINTS PURGE
/

CREATE TABLE TELS_CWP_CUSTOMERBILL_STAGE
(
  FILENAME                        VARCHAR2(255 BYTE) NOT NULL,
  BATCHPROCESSDATE                DATE          NOT NULL,
  RECORDNUMBER                    NUMBER(15)    NOT NULL,
  IS_FILTERED                     NUMBER(1)     NOT NULL,
  SOURCE_SYSTEM                   VARCHAR2(255 BYTE),
  SOURCE_SYSTEM_KEY               VARCHAR2(255 BYTE),
  SOURCE_SYS_BILLING_ACCOUNT_KEY  VARCHAR2(255 BYTE),
  BILLING_ACCOUNT_NUMBER          VARCHAR2(255 BYTE),
  INVOICE_NUMBER                  VARCHAR2(255 BYTE),
  TRANSACTION_TYPE                VARCHAR2(255 BYTE),
  INVOICE_TYPE                    VARCHAR2(255 BYTE),
  INVOICE_FROM_DATE               VARCHAR2(255 BYTE),
  INVOICE_TO_DATE                 VARCHAR2(255 BYTE),
  USAGE_BILL_FROM_DATE            VARCHAR2(255 BYTE),
  USAGE_BILL_THROUGH_DATE         VARCHAR2(255 BYTE),
  INVOICE_AMOUNT                  VARCHAR2(255 BYTE),
  BILL_DATE                       VARCHAR2(255 BYTE),
  DUE_DATE                        VARCHAR2(255 BYTE),
  CURRENCY                        VARCHAR2(255 BYTE),
  BALANCE_FORWARD                 VARCHAR2(255 BYTE),
  TOTAL_DUE                       VARCHAR2(255 BYTE),
  INVOICE_COMMENTS                VARCHAR2(255 BYTE),
  VOIDING_EVENT_NUMBER            VARCHAR2(255 BYTE),
  B2B_INSERT_TIMESTAMP            VARCHAR2(255 BYTE),
  B2B_BATCH_ID                    VARCHAR2(255 BYTE),
  B2B_KEY                         VARCHAR2(255 BYTE)
)
TABLESPACE TELS_SOURCES
PCTUSED    0
PCTFREE    10
INITRANS   1
MAXTRANS   255
STORAGE    (
            INITIAL          64K
            NEXT             1M
            MINEXTENTS       1
            MAXEXTENTS       UNLIMITED
            PCTINCREASE      0
            BUFFER_POOL      DEFAULT
           )
LOGGING 
NOCOMPRESS 
NOCACHE
NOPARALLEL
MONITORING
/


CREATE UNIQUE INDEX TELS_STAGE_CUSTOMERBILL_PK ON TELS_CWP_CUSTOMERBILL_STAGE
(FILENAME, BATCHPROCESSDATE, RECORDNUMBER)
LOGGING
TABLESPACE TELS_SOURCES
PCTFREE    10
INITRANS   2
MAXTRANS   255
STORAGE    (
            INITIAL          64K
            NEXT             1M
            MINEXTENTS       1
            MAXEXTENTS       UNLIMITED
            PCTINCREASE      0
            BUFFER_POOL      DEFAULT
           )
NOPARALLEL
/

...
#<<tels_outbound_aurora.sh>>

#-------------------------------------------------------------------------------
# Date        Author          Version  Comments
#-------------------------------------------------------------------------------
# 25/05/2018  Andrew Mills    v00
#
# Description:
#   File handler script to generate the outbound Aurora Pre-Calc and
#     Post-Approved files.
#
# Invoked By: Trigger File
#   Name: TELS_AURORATRIG_<DEV|TST|UAT|PRD>.txt
#   Parameters: Period Name
#
# Command Line: tels_outbound_aurora.sh <Trigger File Name>
#
################################################################################
...
echo "[CallScript-OutboundAurora]"
echo "[CallScript-OutboundAurora]  === [$timestamp]:: START:: $filehandler ]===" 
echo "[CallScript-OutboundAurora]"

echo "[CallScript-OutboundAurora] Current FileHandler path: " $filehandlername
echo "[CallScript-OutboundAurora] Current FileHandler: " $filehandler
echo "[CallScript-OutboundAurora] $tenantid_uc Supplied Outbound file name : " $filename
echo "[CallScript-OutboundAurora] Current SourceFileType: " $sourcetype
echo "[CallScript-OutboundAurora] $tenantid_uc Outbound filetype: " $filetype
echo "[CallScript-OutboundAurora] [$filename] received from $tenantid_uc, and it has [ $filerecords ] records."
...
if [ $srcfiletype = "TELS_AURORATRIG" ]; then
    parameterfile="tels_outbound_aurora_parm.txt"
fi
...
echo "[CallScript-OutboundAurora] Parameter file $parameterfile generated and moved to $infasrcdir. "
...
echo "[CallScript-OutboundAurora] Source & File Type is: $srcfiletype"
if [ $srcfiletype = "TELS_AURORATRIG" ]; then
  curr_dir=`pwd`
  cd $infatgtdir
  ...
  cd $curr_dir
  echo "[CallScript-OutboundAurora] Output File Name - Header: $OutputFileName1"
  echo "[CallScript-OutboundAurora] Output File Name - Detail: $OutputFileName2"
  echo "[CallScript-OutboundAurora] Output File Name - Footer: $OutputFileName3"
fi
...
echo "[CallScript-OutboundAurora] Calling Utility function to check size and move files to AppServer."
...
echo "[CallScript-OutboundAurora]"
echo "[CallScript-OutboundAurora]  === [$timestamp]::END:: $filehandler ] ==="

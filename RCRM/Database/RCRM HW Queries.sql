...
FROM 
  ...
  LEFT OUTER JOIN HW_ASSOC_CONTRACT_OLI haco ON haco.ORDER_NUMBER = ho.ORDER_NUMBER AND haco.STATUS_HEADER = ho.STATUS_HEADER AND haco.ROOT_ITEM_ROW_ID = ho.ROOT_ITEM_ROW_ID-- AND haco.FILENAME = ho.FILENAME 
                                                AND EXISTS (SELECT 1 FROM TELS_RCRM_MV_ATTRIBUTE ATT WHERE ATT.ROW_ID = haco.ROW_ID AND ATT.ATTRIBUTE_VALUE = ho.PART_NUMBER)-- AND ATT.FILENAME = haco.FILENAME
  ...
  LEFT OUTER JOIN ATTRIBUTES              a1 ON a1.ATTRIBUTE_OWNER_ROW_ID = haco.ROW_ID AND a1.ORDER_NUMBER = haco.ORDER_NUMBER AND a1.ATTRIBUTE_DISPLAY_NAME = 'Device Max. Retail Price'-- AND a1.FILENAME = haco.FILENAME
  LEFT OUTER JOIN ATTRIBUTES              a2 ON a2.ATTRIBUTE_OWNER_ROW_ID = ho.ROW_ID AND a2.ORDER_NUMBER = ho.ORDER_NUMBER AND a2.ATTRIBUTE_DISPLAY_NAME = 'Mobile Device Payment Method' AND a2.ATTRIBUTE_ACTION_CODE <> 'Delete' -- AND a2.FILENAME = ho.FILENAME
  LEFT OUTER JOIN ATTRIBUTES              a3 ON a3.ATTRIBUTE_OWNER_ROW_ID = ho.ROW_ID AND a3.ORDER_NUMBER = ho.ORDER_NUMBER AND a3.ATTRIBUTE_DISPLAY_NAME = 'IMEI' AND a3.ATTRIBUTE_ACTION_CODE <> 'Delete'-- AND a3.FILENAME = ho.FILENAME
  LEFT OUTER JOIN ATTRIBUTES              a4 ON a4.ATTRIBUTE_OWNER_ROW_ID = haco.ROW_ID AND a4.ORDER_NUMBER = haco.ORDER_NUMBER AND a4.ATTRIBUTE_DISPLAY_NAME = 'Contract Term'-- AND a4.FILENAME = haco.FILENAME
  LEFT OUTER JOIN ATTRIBUTES              a5 ON a5.ATTRIBUTE_OWNER_ROW_ID = haco.ROW_ID AND a5.ORDER_NUMBER = haco.ORDER_NUMBER AND a5.ATTRIBUTE_DISPLAY_NAME IN('MRO Amount','Device Commitment','Device Purchase Price')-- AND a5.FILENAME = haco.FILENAME
  LEFT OUTER JOIN ATTRIBUTES              a6 ON a6.ATTRIBUTE_OWNER_ROW_ID = hamo.ROW_ID AND a6.ORDER_NUMBER = hamo.ORDER_NUMBER AND a6.ATTRIBUTE_DISPLAY_NAME = 'MRO Amount'-- AND a6.FILENAME = hamo.FILENAME

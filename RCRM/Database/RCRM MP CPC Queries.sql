...
FROM 
  ...
  LEFT OUTER JOIN MP_ASSOC_HW_OLI             maho ON maho.ORDER_NUMBER = mo.ORDER_NUMBER AND maho.STATUS_HEADER = mo.STATUS_HEADER AND maho.ROOT_ITEM_ROW_ID = mo.ROOT_ITEM_ROW_ID AND maho.PRODUCT <> 'ARO Accessory'-- AND maho.FILENAME = mo.FILENAME
  LEFT OUTER JOIN MP_ASSOC_CONTRACT_ADD_OLI   mcao ON mcao.ORDER_NUMBER = mo.ORDER_NUMBER AND mcao.STATUS_HEADER = mo.STATUS_HEADER AND mcao.ROOT_ITEM_ROW_ID = mo.ROOT_ITEM_ROW_ID AND mcao.PRODUCT IN('Device Lease Contract','Device Payment Contract')-- AND mcao.FILENAME = mo.FILENAME
  ...
  LEFT OUTER JOIN ATTRIBUTES                    a1 ON a1.ATTRIBUTE_OWNER_ROW_ID = mmcdo.ROW_ID AND a1.ORDER_NUMBER = mmcdo.ORDER_NUMBER AND a1.ATTRIBUTE_DISPLAY_NAME = 'Contract Term'-- AND a1.FILENAME = mmcdo.FILENAME
  LEFT OUTER JOIN ATTRIBUTES                    a2 ON a2.ATTRIBUTE_OWNER_ROW_ID = mmcao.ROW_ID AND a2.ORDER_NUMBER = mmcao.ORDER_NUMBER AND a2.ATTRIBUTE_DISPLAY_NAME = 'Contract Term'-- AND a2.FILENAME = mmcao.FILENAME
  LEFT OUTER JOIN ATTRIBUTES                    a3 ON a3.ATTRIBUTE_OWNER_ROW_ID = maso.ROW_ID AND a3.ORDER_NUMBER = maso.ORDER_NUMBER AND a3.ATTRIBUTE_DISPLAY_NAME = 'Network Type'-- AND a3.FILENAME = maso.FILENAME
  LEFT OUTER JOIN ATTRIBUTES                    a4 ON a4.ATTRIBUTE_OWNER_ROW_ID = mmcao.ROW_ID AND a4.ORDER_NUMBER = mmcao.ORDER_NUMBER AND a4.ATTRIBUTE_DISPLAY_NAME = 'Minimum Spend'-- AND a4.FILENAME = mmcao.FILENAME
  LEFT OUTER JOIN ATTRIBUTES                    a5 ON a5.ATTRIBUTE_OWNER_ROW_ID = mmcdo.ROW_ID AND a5.ORDER_NUMBER = mmcdo.ORDER_NUMBER AND a5.ATTRIBUTE_DISPLAY_NAME = 'Plan ID'-- AND a5.FILENAME = mmcdo.FILENAME
  LEFT OUTER JOIN ATTRIBUTES                    a6 ON a6.ATTRIBUTE_OWNER_ROW_ID = mmcdo.ROW_ID AND a6.ORDER_NUMBER = mmcdo.ORDER_NUMBER AND a6.ATTRIBUTE_DISPLAY_NAME = 'Minimum Spend'-- AND a6.FILENAME = mmcdo.FILENAME
  LEFT OUTER JOIN ATTRIBUTES                    a7 ON a7.ATTRIBUTE_OWNER_ROW_ID = mmpdo.ROW_ID AND a7.ORDER_NUMBER = mmpdo.ORDER_NUMBER AND a7.ATTRIBUTE_DISPLAY_NAME = 'Minimum Spend'-- AND a7.FILENAME = mmpdo.FILENAME 
  LEFT OUTER JOIN ATTRIBUTES                    a8 ON a8.ATTRIBUTE_OWNER_ROW_ID = mafo.ROW_ID AND a8.ORDER_NUMBER = mafo.ORDER_NUMBER AND a8.ATTRIBUTE_DISPLAY_NAME = 'Supplied in Store?'-- AND a8.FILENAME = mafo.FILENAME
  LEFT OUTER JOIN ATTRIBUTES                    a9 ON a9.ATTRIBUTE_OWNER_ROW_ID = mcao.ROW_ID AND a9.ORDER_NUMBER = mcao.ORDER_NUMBER AND a9.ATTRIBUTE_DISPLAY_NAME = 'Plan ID' AND a9.ATTRIBUTE_VALUE = mo.PART_NUMBER-- AND a9.FILENAME = mcao.FILENAME
  LEFT OUTER JOIN ATTRIBUTES                   a10 ON a10.ATTRIBUTE_OWNER_ROW_ID = a9.ATTRIBUTE_OWNER_ROW_ID AND a10.ORDER_NUMBER = a9.ORDER_NUMBER AND a10.ATTRIBUTE_DISPLAY_NAME = 'Redeemed' AND a10.ATTRIBUTE_VALUE IN('Yes','DLC Redeemed','DPC Redeemed')-- AND a10.FILENAME = a9.FILENAME

...
FROM 
  ...
  LEFT OUTER JOIN MP_ASSOC_SERVICE_MOLI       masm_i ON masm_i.ORDER_NUMBER = mpam.ORDER_NUMBER AND masm_i.STATUS_HEADER = mpam.STATUS_HEADER AND masm_i.PROMOTION_INTEGRATION_ID = mpam.PROMOTION_INTEGRATION_ID AND masm_i.PRODUCT = 'Internet Service'
  LEFT OUTER JOIN MP_ASSOC_SERVICE_MOLI       masm_v ON masm_v.ORDER_NUMBER = mpam.ORDER_NUMBER AND masm_v.STATUS_HEADER = mpam.STATUS_HEADER AND masm_v.PROMOTION_INTEGRATION_ID = mpam.PROMOTION_INTEGRATION_ID AND masm_v.PRODUCT = 'Voice Service'
  LEFT OUTER JOIN SERVICE_ASSOC_ACCESS_TYPE_OLI sato_i ON sato_i.ORDER_NUMBER = masm_i.ORDER_NUMBER AND sato_i.STATUS_HEADER = masm_i.STATUS_HEADER AND sato_i.ROOT_ITEM_ROW_ID = masm_i.ROOT_ITEM_ROW_ID AND sato_i.ACTION_CODE IN('Add','-','Update')
  LEFT OUTER JOIN SERVICE_ASSOC_ACCESS_TYPE_OLI sato_v ON sato_v.ORDER_NUMBER = masm_v.ORDER_NUMBER AND sato_v.STATUS_HEADER = masm_v.STATUS_HEADER AND sato_v.ROOT_ITEM_ROW_ID = masm_v.ROOT_ITEM_ROW_ID AND sato_v.ACTION_CODE IN('Add','-','Update')
  ...
  LEFT OUTER JOIN ATTRIBUTES                    a3 ON a3.ATTRIBUTE_OWNER_ROW_ID = mafo.ROW_ID AND a3.ORDER_NUMBER = mafo.ORDER_NUMBER AND a3.ATTRIBUTE_DISPLAY_NAME LIKE 'Supplied in Store%'
  LEFT OUTER JOIN ATTRIBUTES                   a3i ON a3i.ATTRIBUTE_OWNER_ROW_ID = mafo.ROW_ID AND a3i.ORDER_NUMBER = mafo.ORDER_NUMBER AND a3i.ATTRIBUTE_DISPLAY_NAME = 'Hardware Supplied'
...
SELECT * FROM TELS_RCRM_ORDERS_VIEW WHERE ORDER_NUMBER = '1-1781214299379' ORDER BY REC_TYPE;
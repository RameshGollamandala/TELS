...
#   standard ODI Org files.
#
# Invoked By: LND Autoloader based on the mapping of file-type to file handler
#   script name.
#
# Command Line: tels_inbound_org.sh <inbound filename without path>
#
################################################################################
...
#############################################
target_archive_dir=$PIMS_User_targets_archive
target_badfiles_dir=$PIMS_User_targets_badfiles
...
if [ $srcfiletype = "BULK_DLR" ]; then
    OutputFileName1=$tenantid_uc"_OGPT_"$custinst_uc"_"$buname"_"$srcfiletype"_"$timestamp".txt"
	OutputFileName2=$tenantid_uc"_OGPO_"$custinst_uc"_"$buname"_"$srcfiletype"_"$timestamp".txt"
	OutputFileName3=$tenantid_uc"_CLCU_"$custinst_uc"_"$buname"_"$srcfiletype"_"$timestamp".txt"
	OutputFileName4=$tenantid_uc"_ERROR_"$custinst_uc"_"$buname"_"$srcfiletype"_"$timestamp".txt"
elif [ $srcfiletype = "PIMS_USER" ]; then	
    OutputFileName1=$tenantid_uc"_OGPT_"$custinst_uc"_"$buname"_"$srcfiletype"_"$timestamp".txt"
fi

ErrorFileName=$tenantid_uc"_ERROR_"$custinst_uc"_"$buname"_"$srcfiletype"_"$timestamp".txt"
...
if [ $srcfiletype = "BULK_DLR" -o $srcfiletype = "PIMS_USER" ]; then
...
if [ $srcfiletype = "BULK_DLR" -o $srcfiletype = "PIMS_USER" ]; then
    ...
    if [ "$OutputFileName4" != "" ]; then
		...
		echo "<b><font size="4" color="red">$OutputFileName4 contains $filereccount4 records</font></b>"
